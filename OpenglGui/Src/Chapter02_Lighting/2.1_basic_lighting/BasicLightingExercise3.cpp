...
BasicLightingExercise3::BasicLightingExercise3(QWidget *parent)
    : GLWidget(parent),
      ...
      m_camera(this)
{
    ...
    connect(m_sliderAmbientStrength, &QSlider::valueChanged, this, static_cast<void (BasicLightingExercise3::*)()>(&BasicLightingExercise3::update));
    connect(m_sliderSpecularStrength, &QSlider::valueChanged, this, static_cast<void (BasicLightingExercise3::*)()>(&BasicLightingExercise3::update));
    connect(m_sliderSpecularShininess, &QSlider::valueChanged, this, static_cast<void (BasicLightingExercise3::*)()>(&BasicLightingExercise3::update));
    m_timer.start(18);
    connect(&m_timer, &QTimer::timeout, this, &BasicLightingExercise3::handleTimeout);
}

BasicLightingExercise3::~BasicLightingExercise3()
{
    makeCurrent();
...
void BasicLightingExercise3::initializeGL()
{
    if (!context())
    {
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/01_Basic_Lighting/basic_lighting_exercise3.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/01_Basic_Lighting/basic_lighting_exercise3.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
    ...
    //    m_camera.setYawAngle(-91.78);
    //    m_camera.setPitchAngle(-0.37);
}

void BasicLightingExercise3::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void BasicLightingExercise3::paintGL()
{
...
bool BasicLightingExercise3::event(QEvent *e)
{
    m_camera.handle(e);
    return QWidget::event(e);
}

void BasicLightingExercise3::handleTimeout()
{
