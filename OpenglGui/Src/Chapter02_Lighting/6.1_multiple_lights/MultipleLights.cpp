...
void MultipleLights::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/05_multiple_lights/multiple_lights.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/05_multiple_lights/multiple_lights.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
