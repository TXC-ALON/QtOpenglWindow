...
void Transformations::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/01_Bird/05_transformations/transformations.vert");
        if (!result)
        {
            qDebug() << m_program.log();
        }
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/01_Bird/05_transformations/transformations.frag");
        if (!result)
        {
            qDebug() << m_program.log();
        }
...
void Transformations::mouseMoveEvent(QMouseEvent *event)
{
    ...
    {
        m_transform.translate(dx / 800, -dy / 200, 0);
        update();
    }
