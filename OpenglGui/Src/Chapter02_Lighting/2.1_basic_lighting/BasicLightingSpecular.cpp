...
BasicLightingSpecular::BasicLightingSpecular(QWidget *parent)
    : GLWidget(parent),
      ...
      m_camera(this)
{
    ...
    connect(m_sliderAmbientStrength, &QSlider::valueChanged, this, static_cast<void (BasicLightingSpecular::*)()>(&BasicLightingSpecular::update));
    connect(m_sliderSpecularStrength, &QSlider::valueChanged, this, static_cast<void (BasicLightingSpecular::*)()>(&BasicLightingSpecular::update));
    connect(m_sliderSpecularShininess, &QSlider::valueChanged, this, static_cast<void (BasicLightingSpecular::*)()>(&BasicLightingSpecular::update));
}
...
void BasicLightingSpecular::initializeGL()
{
    if (!context())
    {
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/01_Basic_Lighting/basic_lighting.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/01_Basic_Lighting/basic_lighting.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
...
void BasicLightingSpecular::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void BasicLightingSpecular::paintGL()
{
...
bool BasicLightingSpecular::event(QEvent *e)
{
    m_camera.handle(e);
