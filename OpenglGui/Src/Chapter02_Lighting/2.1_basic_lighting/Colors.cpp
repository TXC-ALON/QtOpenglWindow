...
void Colors::initializeGL()
{
    ...
    bool result = true;
    result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/01_Basic_Lighting/colors.vert");
    if (!result)
    {
        qDebug() << m_lightShader.log();
    }
    result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/01_Basic_Lighting/colors.frag");
    if (!result)
    {
        qDebug() << m_lightShader.log();
    }
    ...
    // 相机类初始化
    m_camera.init();
    m_camera.setCameraPos({1.6, 1.3, 5});
    m_camera.setYawAngle(-105);
    m_camera.setPitchAngle(-4.5);
}
