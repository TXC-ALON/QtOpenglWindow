...
void LightCastersSpot::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/04_Light_Casters/light_casters_spot.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/04_Light_Casters/light_casters_spot.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
...
void LightCastersSpot::paintGL()
{
    ...
    m_lightShader.setUniformValue("light.cutOff", GLfloat(cos(m_sliderCutOff * 3.1415926 / 180)));
    m_lightShader.setUniformValue("light.outerCutOff", GLfloat(cos(m_sliderOuterCutOff * 3.1415926 / 180)));
    ...
    {
        m_lightShader.setUniformValue("light.outerCutOff", GLfloat(cos(m_sliderCutOff * 3.1415926 / 180)));
    }
...
QWidget *LightCastersSpot::createControlPanel()
{
    ...
    QVBoxLayout *layout = new QVBoxLayout(panel);
    {
        ...
        connect(pPositionSpinbox, &PositionSpinbox::spinbox_valueChanged, this, &LightCastersSpot::onPositionChanged);
    }
    // 创建ComboBox
    ...
    connect(comboBox, &QComboBox::currentIndexChanged, this, &LightCastersSpot::onDistanceChanged);
    onDistanceChanged(0);
    ...
    connect(m_checkBox, &QCheckBox::stateChanged, this, &LightCastersSpot::onSoftButtonChanged);
...
void LightCastersSpot::onPositionChanged()
{
...
QMap<int, LightCastersSpot::Data> LightCastersSpot::prepare_data()
{
    QMap<int, Data> data;
    data[7] = {1.0, 0.7, 1.8};
...
void LightCastersSpot::onDistanceChanged(int index)
{
...
void LightCastersSpot::onSliderValueChanged()
{
...
void LightCastersSpot::onSoftButtonChanged(int state)
{
    if (state == Qt::Checked)
    {
