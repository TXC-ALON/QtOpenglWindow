...
Blending_sort::Blending_sort(QWidget *parent)
    : GLWidget(parent),
...
void Blending_sort::initializeGL()
{
    if (!context())
    {
    ...
    {
        bool result = true;
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/04_Advanced/03_Blending/blending_sort.vert");
        if (!result)
        {
            qDebug() << m_program.log();
        }
        result = m_program.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/04_Advanced/03_Blending/blending_sort.frag");
        if (!result)
        {
            qDebug() << m_program.log();
        }
    ...
    m_texture[2] = std::make_unique<QOpenGLTexture>(QImage(":/Resources/textures/window.png"));
    for (int i = 0; i < 2; ++i)
    {
        m_texture[i]->create();
        // 纹理环绕方式
...
void Blending_sort::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void Blending_sort::paintGL()
{
    ...
    m_transparentVAO.bind();
    m_texture[2]->bind();
    // sort distance from far to near
    if (use_sort)
    {
        auto cameraPos = m_camera.getCameraPos();
        std::sort(m_windowPos.begin(), m_windowPos.end(), [&](QVector3D &a, QVector3D &b)
                  { return a.distanceToPoint(cameraPos) > b.distanceToPoint(cameraPos); });
    }
    else
    {
        m_windowPos = {
    ...
    for (int i = 0; i < m_windowPos.size(); ++i)
    {
        m_model.setToIdentity();
        m_model.translate(m_windowPos[i]);
        m_program.setUniformValue("model", m_model);
        glDrawArrays(GL_TRIANGLES, 0, 6);
    }
...
bool Blending_sort::event(QEvent *e)
{
    m_camera.handle(e);
    return QWidget::event(e);
}

QWidget *Blending_sort::createControlPanel()
{
    ...
    connect(radioGroup, &QButtonGroup::buttonClicked, this, &Blending_sort::onSwitchSort_slot);

    return controlPanel;
}

void Blending_sort::onSwitchSort_slot(QAbstractButton *button)
{
