...
void ShadersInterpolation::initializeGL()
{
    ...
    // shader初始化
    {
        bool result = true;
        // uniform
        result = m_program[0].addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/01_Bird/03_shaders_interpolation/shaders_interpolation.vert");
        if (!result)
        {
            qDebug() << m_program[0].log();
        }
        result = m_program[0].addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/01_Bird/03_shaders_interpolation/shaders_interpolation.frag");
        if (!result)
        {
            qDebug() << m_program[0].log();
        }
        ...
        // no uniform
        result = m_program[1].addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/01_Bird/03_shaders_interpolation/shaders_interpolation1.vert");
        if (!result)
        {
            qDebug() << m_program[1].log();
        }
        result = m_program[1].addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/01_Bird/03_shaders_interpolation/shaders_interpolation1.frag");
        if (!result)
        {
            qDebug() << m_program[1].log();
        }
...
void ShadersInterpolation::paintGL()
{
    ...
    randomDouble = randomGenerator->generateDouble();
    // float greenValue = sin(randomDouble * 2 * M_PI) / 2.0 + 0.5;
    float greenValue = sin(time) / 2.0f + 0.5f;
    // float greenValue = std::sin(randomDouble) / 2.0 + 0.5;
    //  qDebug() << greenValue;
