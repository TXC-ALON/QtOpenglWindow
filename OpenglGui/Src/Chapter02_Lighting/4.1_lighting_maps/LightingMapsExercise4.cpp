...
void LightingMapsExercise4::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/03_Lighting_Maps/lighting_maps_exercise4.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/03_Lighting_Maps/lighting_maps_exercise4.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
