...
void LightingMapsSpecularMap::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/03_Lighting_Maps/lighting_maps_specular_map.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/03_Lighting_Maps/lighting_maps_specular_map.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
