...
BasicLightingDiffuse::BasicLightingDiffuse(QWidget *parent)
    : GLWidget(parent),
...
void BasicLightingDiffuse::initializeGL()
{
    if (!context())
    {
    ...
    // shader初始化
    {
        ...
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/01_Basic_Lighting/basic_lighting_disffuse.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
    ...
    m_camera.setCameraPos({0, 0, 3});
}

void BasicLightingDiffuse::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void BasicLightingDiffuse::paintGL()
{
...
bool BasicLightingDiffuse::event(QEvent *e)
{
    m_camera.handle(e);
