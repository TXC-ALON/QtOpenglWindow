...
LightingMapsDiffuseMap::LightingMapsDiffuseMap(QWidget *parent)
    : GLWidget(parent),
...
LightingMapsDiffuseMap::~LightingMapsDiffuseMap()
{
    makeCurrent();
...
void LightingMapsDiffuseMap::initializeGL()
{
    if (!context())
    {
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/03_Lighting_Maps/lighting_maps_diffuse_map.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/03_Lighting_Maps/lighting_maps_diffuse_map.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
...
void LightingMapsDiffuseMap::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void LightingMapsDiffuseMap::paintGL()
{
    ...
    // material
    m_lightShader.setUniformValue("material.specular", QVector3D(0.5, 0.5, 0.5));
...
bool LightingMapsDiffuseMap::event(QEvent *e)
{
    m_camera.handle(e);
