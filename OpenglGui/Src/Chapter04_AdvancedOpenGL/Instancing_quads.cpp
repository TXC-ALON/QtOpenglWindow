...
Instancing_quads::Instancing_quads(QWidget *parent)
    : GLWidget(parent),
      m_camera(this)
{
}

Instancing_quads::~Instancing_quads()
{
    makeCurrent();
...
void Instancing_quads::initializeGL()
{
    if (!context())
    {
    ...
    connect(logger, &QOpenGLDebugLogger::messageLogged, this, &Instancing_quads::handleLoggedMessage);
    logger->startLogging();
    ...
    // shader初始化
    {
        m_shader = std::make_unique<ShaderUtil>(":/Resources/shaders/04_Advanced/08_Instances/instancing.vert",
                                                ":/Resources/shaders/04_Advanced/08_Instances/instancing.frag");
    }
    {
...
void Instancing_quads::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void Instancing_quads::paintGL()
{
...
bool Instancing_quads::event(QEvent *e)
{
    m_camera.handle(e);
    return QWidget::event(e);
}

void Instancing_quads::handleLoggedMessage(const QOpenGLDebugMessage &debugMessage)
{
