...
LightingMapsExercise2::LightingMapsExercise2(QWidget *parent)
    : GLWidget(parent),
...
LightingMapsExercise2::~LightingMapsExercise2()
{
    makeCurrent();
...
void LightingMapsExercise2::initializeGL()
{
    if (!context())
    {
    ...
    // shader初始化
    {
        ...
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/Resources/shaders/02_Lighting/03_Lighting_Maps/lighting_maps_exercise2.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/Resources/shaders/02_Lighting/03_Lighting_Maps/lighting_maps_exercise2.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
...
void LightingMapsExercise2::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void LightingMapsExercise2::paintGL()
{
...
bool LightingMapsExercise2::event(QEvent *e)
{
    m_camera.handle(e);
