...
ModelUtil::ModelUtil(QString path, QOpenGLContext *context)
    : m_context(context), directory(path), m_positiveX(FLT_MIN), m_positiveY(FLT_MIN), m_positiveZ(FLT_MIN), m_negativeX(FLT_MAX), m_negativeY(FLT_MAX), m_negativeZ(FLT_MAX), m_scaleFactor(1.0)
{
...
void ModelUtil::get_model_info(const aiScene *scene)
{
    ...
    if (scene->HasMeshes( ))
    {
        for (unsigned int i = 0; i < scene->mNumMeshes; ++i)
        {
            ...
            {
                const aiVector3D &vec = scene->mMeshes[i]->mVertices[j];
                minVec.x              = std::min(minVec.x, vec.x);
                minVec.y              = std::min(minVec.y, vec.y);
                minVec.z              = std::min(minVec.z, vec.z);
                maxVec.x              = std::max(maxVec.x, vec.x);
                maxVec.y              = std::max(maxVec.y, vec.y);
                maxVec.z              = std::max(maxVec.z, vec.z);
            }
        }
    }

    qDebug( ) << "Total vertices: " << totalVertices;
    qDebug( ) << "Total faces: " << totalFaces;
    qDebug( ) << "Bounding box min: " << minVec.x << ", " << minVec.y << ", " << minVec.z;
    qDebug( ) << "Bounding box max: " << maxVec.x << ", " << maxVec.y << ", " << maxVec.z;
}
...
{
    if (m_positiveX == FLT_MIN || m_positiveY == FLT_MIN || m_positiveZ == FLT_MIN || m_negativeX == FLT_MAX || m_negativeY == FLT_MAX || m_negativeZ == FLT_MAX)
    {
        return;
    }
