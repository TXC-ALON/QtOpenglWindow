...
CameraCircle::CameraCircle(QWidget *parent)
    : GLWidget(parent),
      ...
      m_cameraUp(0.0, 1.0, 0.0)
{
    ...
    m_timer.start(100);
    connect(&m_timer, &QTimer::timeout, this, &CameraCircle::handleTimeout);
}
...
void CameraCircle::initializeGL()
{
    if (!context())
    {
...
void CameraCircle::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}

void CameraCircle::paintGL()
{
...
void CameraCircle::handleTimeout()
{
    // look at
    float radius = 10.0;
    ...
    m_cameraPos.setX(camX);
    m_cameraPos.setZ(camZ);
