...
void Materials::initializeGL()
{
    ...
    {
        bool result = true;
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Vertex, ":Resources/shaders/02_Lighting/02_Materials/materials.vert");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
        result = m_lightShader.addShaderFromSourceFile(QOpenGLShader::Fragment, ":Resources/shaders/02_Lighting/02_Materials/materials.frag");
        if (!result)
        {
            qDebug() << m_lightShader.log();
        }
    ...
    // 相机类初始化
    m_camera.init();
    m_camera.setCameraPos({0.0, 0.0, 5.0});
